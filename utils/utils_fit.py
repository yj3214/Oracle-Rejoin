...
def fit_one_epoch(model_train, model, loss,optimizer, epoch, epoch_step, epoch_step_val, gen, genval, Epoch, cuda, log_dir):
    total_loss      = 0
    total_accuracy  = 0
    ...
    with tqdm(total=epoch_step,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            # a = outputs.sum(1)
            outputs = nn.Sigmoid()(model_train(images).sum(1))
    ...
    with tqdm(total=epoch_step_val, desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(genval):
            ...
            with torch.no_grad():
                ...
                optimizer.zero_grad()
                outputs = nn.Sigmoid()(model_train(images_val).sum(1))
                output  = loss(outputs, targets_val)

                equal       = torch.eq(torch.round(outputs), targets_val)       #4舍5入取整
                accuracy    = torch.mean(equal.float())
    ...
    torch.save(model.state_dict(), log_dir + '/ep%03d-loss%.4f-val_loss%.4f.pth'%((epoch + 1), total_loss / epoch_step, val_loss / epoch_step_val))
